    ...
    return (
        <div className="min-h-screen bg-[#e5e7eb] relative flex items-center justify-center p-4 lg:p-8 font-sans overflow-hidden">
            ...
            <div className="w-full max-w-5xl relative z-10 animate-fade-slide-up">
                ...
                <div className="bg-white/20 backdrop-blur-[50px] rounded-[2rem] border border-white/50 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col lg:flex-row ring-1 ring-white/30">
                    ...
                    <div className="flex-1 p-8 lg:p-16 flex flex-col justify-center bg-white/5 animate-fade-in delay-500">
                        ...
                        {error && (
                            <div className="mb-6 p-4 bg-red-500/10 backdrop-blur-md text-red-600 rounded-xl border border-red-500/20 font-black text-sm animate-shake">
                                {error}
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-1 group">
                                ...
                                <input
                                    ...
                                    className="w-full px-6 py-4 bg-white/20 backdrop-blur-2xl border border-white/50 rounded-xl focus:outline-none focus:ring-4 focus:ring-orange-500/20 focus:border-orange-500/50 transition-all font-bold text-gray-900 placeholder:text-gray-400/60 shadow-[inset_0_1px_2px_rgba(255,255,255,0.3)]"
                                    required
                                />
                            </div>
                            ...
                            <div className="flex items-center justify-between px-1">
                                ...
                                >
                                    <div className={`w-10 h-5 rounded-full relative transition-all duration-300 ${remember ? 'bg-orange-600 shadow-[0_0_15px_rgba(249,115,22,0.4)]' : 'bg-white/20 border border-white/40'}`}>
                                        <div className={`absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 rounded-full bg-white transition-all duration-300 shadow-sm ${remember ? 'left-5.5' : 'left-1'}`}></div>
                                    </div>
                                    <span className={`text-[11px] font-black uppercase tracking-wider transition-colors ${remember ? 'text-gray-900' : 'text-gray-400'}`}>Ingat Saya</span>
                                </button>
                                <button type="button" className="text-[11px] font-black text-orange-600 hover:text-orange-500 transition-colors uppercase tracking-wider">Lupa Password?</button>
                            </div>

                            <button
                                type="submit"
                                disabled={loading}
                                className={`w-full py-4 lg:py-5 bg-orange-600 text-white font-black text-lg rounded-xl shadow-2xl shadow-orange-600/30 hover:bg-orange-500 hover:shadow-orange-600/50 hover:-translate-y-1 active:scale-95 transition-all flex items-center justify-center space-x-3 group ${loading ? 'opacity-70' : ''}`}
                            >
                                {loading ? (
